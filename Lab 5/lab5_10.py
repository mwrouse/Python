"""
    ...
    Description: generates two random positive integers and displays them in a number sentence as strings and asks for the answer then checks to make sure the answer is correct. Asks
                 user if they want to replay. Displays numSent1 every other time. Tells user how many problems they got correct and incorrect, if all are correct they get a special messege
"""
import random

counter = 0 #counter variable to determine which number sentence to display
...
while userInput.lower() != "no" and userInput.lower() != "n":
    ...
    if counter % 2 == 0: #if counter is divisible by two (displays every other time)
        userAnswer = int(input("\nOkay, " + userName + " what is the answer to " + numSent1))
    else:
        userAnswer = int(input("\nOkay, " + userName+ " what is the answer to " + numSent2))
...
if correct == counter:
    print("\nCongratulations,", userName, "! You get a GOLD STAR!")
